...
func PutCityData(url string) error {
	...
	value, err := model.Pool.ZAdd("ruoai_city_spider", redis.Z{Score: 10, Member: url}).Result()
	fmt.Println("ruoai,城市url添加", url, err)

	if value == 1 { //说明没有这个key
		onlyid := model.WxHead + model.GetKey(16)
		//存对应的data到有序集合
		value, err := model.Pool.ZAdd("ruoai_get_data", redis.Z{Score: 10, Member: onlyid}).Result()
...
func GetCityData() (url_list []string, err error) {
	...
	vals, err := model.Pool.ZRangeByScoreWithScores("ruoai_get_data", redis.ZRangeBy{
		Min:    "0",
		Max:    "50",
		Offset: 0,
		Count:  1,
	...
	for _, value := range vals {
		...
		//删除集合中的一个指定元素
		model.Pool.ZRem("ruoai_get_data", key)
	}
